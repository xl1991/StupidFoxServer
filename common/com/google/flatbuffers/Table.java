...
public class Table {
     ...
     *
     * @param offset
     *            An `int` index into the Table's ByteBuffer containing the
     *            relative offset.
     ...
     * @param offset
     *            An `int` index into the Table's ByteBuffer.
     * @return Returns the length of the vector whose offset is stored at
     *         `offset`.
     */
    ...
    protected int __vector(int offset) {
        offset += bb_pos;
        return offset + bb.getInt(offset) + SIZEOF_INT; // data starts after the
                                                        // length
    }
     ...
     *
     * This is efficient, since it only allocates a new bytebuffer object, but
     * does not actually copy the data, it still refers to the same bytes as the
     ...
     *
     * @param t
     *            A `Table`-derived type that should point to the union at
     *            `offset`.
     * @param offset
     *            An `int` index into the Table's ByteBuffer.
     ...
     *
     * @param bb
     *            A `ByteBuffer` to check if it contains the identifier `ident`.
     * @param ident
